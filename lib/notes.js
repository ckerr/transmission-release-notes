...
class Commit {
  constructor (hash, owner, repo) {
    ...
    this.prKeys = new Map(); // Map<string,GHKey>
...
class Pool {
  constructor () {
    ...
    this.users = {}; // login => octokit user object
  }
}
...
const parseCommitMessage = (commitMessage, commit) => {
  ...
  if ((match = subject.match(/^(.*)\s\(#(\d+)\)$/))) {
    const new_key = new GHKey(owner, repo, parseInt(match[2]));
    commit.prKeys.set(new_key.to_string(), new_key);
    subject = match[1];
  }
  ...
  if ((match = subject.match(/^Merge pull request #(\d+) from (.*)$/))) {
    const new_key = new GHKey(owner, repo, parseInt(match[1]));
    commit.prKeys.set(new_key.to_string(), new_key);
  }
  ...
  if ((match = commitMessage.match(backportPattern))) {
    ...
    const new_key = new GHKey(owner, repo, backportNumber);
    commit.prKeys.set(new_key.to_string(), new_key);
  }
...
const getUserCacheFilename = login => `user-${login}`;
...
const getUser = async (login) => {
  const filename = getUserCacheFilename(login);
  const retryableFunc = () => octokit.users.getByUsername({ username: login });
  return checkCache(filename, () => runRetryable(retryableFunc, MAX_FAIL_COUNT));
};
...
const getReviews = async (ghKey) => {
  const { number, owner, repo } = ghKey;
  const name = `${owner}-${repo}-issue-${number}-reviews`;
  const retryableFunc = () => octokit.pulls.listReviews({ pull_number: number, owner, repo, per_page: 100 });
  return checkCache(name, () => runRetryable(retryableFunc, MAX_FAIL_COUNT));
};
...
const addRepoToPool = async (pool, repo, from, to) => {
  ...
  for (const commit of commits) {
    ...
    for (const pull of (await getCommitPulls(owner, repo, hash)).data) {
      const new_key = GHKey.NewFromPull(pull);
      commit.prKeys.set(new_key.to_string(), new_key);
    }
  }
  ...
  for (const commit of commits) {
    ...
    for (prKey of commit.prKeys.values()) {
      ...
      await getUser(pull.data.user.login);

      for (const review of (await getReviews(prKey)).data) {
        await getUser(review.user.login);
      }
    }
  }
};
...
const getNotes = async (fromRef, toRef, newVersion) => {
  ...
  const pool = new Pool();
  const toBranch = await getBranchNameOfRef(toRef, SRC_DIR);

  console.log(`Generating release notes between '${fromRef}' and '${toRef}' for version '${newVersion}' in branch '${toBranch}'`);

  // get the electron/electron commits
  const repo = { owner: 'transmission', repo: 'transmission', dir: SRC_DIR };
  await addRepoToPool(pool, repo, fromRef, toRef);
