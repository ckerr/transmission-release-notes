...
const getUserCacheFilename = pull => `${pull.base.repo.owner.login}-${pull.base.repo.name}-user-${pull.user.login}`;
...
const getUser = async (ghKey) => {
  const pr = await getPullRequest(ghKey);
  const filename = getUserCacheFilename(pr.data);
  const retryableFunc = () => octokit.users.getByUsername({ username: pr.data.user.login });
