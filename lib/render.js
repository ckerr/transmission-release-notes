...
const NoteTakers = new Set([ ...DoNotThank, 'GaryElshaw', 'nevack', 'tearfur' ]);
...
function get_arg(argv, key, default_value = undefined) {
  const prefix = `--${key}=`;
  const value = argv.find((str) => str.startsWith(prefix))?.substring(prefix.length);
...
async function main () {
  ...
  const version = get_arg(process.argv, 'version');
  if (!version) {
    console.error(`Missing parameter: version`);
    return;
  }
  ...
  const lede = get_arg(process.argv, 'lede') || fs.readFileSync(0, 'utf-8');
  if (lede) {
    console.log(lede);
    console.log('');
  }
  ...
  console.log('');
  renderHighlights(pulls.filter((pull) => pull.data.labels.some((label) => label.name === 'notes:highlight')));
  renderComponents();
  printThanks(pulls);
}
