...
function getNotesFromParagraph(text) {
  ...
  if (NoNotes.has(notes.toLowerCase())) return undefined;
  return notes;
}
...
const formatPull = (pull) => {
  ...
  return summary ? `${summary} ([#${pull.data.number}](${pull.data.html_url}))` : undefined;
}
...
function printComponent(component) {
  const lines = component.pulls.map(formatPull).filter((line) => !!line);
