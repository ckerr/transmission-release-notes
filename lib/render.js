...
const loginCanWriteNotes = (login) => OrgMembers.has(login);
...
// Highlights go first, then sort by type, then by text
function compareItemForComponent(entry_1, entry_2) {
  ...
  // secondary key: sort by label
  return summary_1.localeCompare(summary_2);
}
  ...
  for (const component of Components) {
    ...
    const logins = new Set(
      [...login_pulls.keys(), ...component_reviewers.keys()]
        .filter((login) => !OrgMembers.has(login))
        .sort());
